...
add_filter(
	...
	function ($gateways) {
		...
		$title = edd_get_option('zc_edd_title', __('USDT, USDC, Binance Pay', 'zeno-crypto-checkout-for-easy-digital-downloads'));
		$title = is_string($title) ? trim($title) : __('USDT, USDC, Binance Pay', 'zeno-crypto-checkout-for-easy-digital-downloads');

		$gateways[ZCEDD_GATEWAY_ID] = array(
...
add_filter(
	...
	function ($icons) {
		$api_key = zc_edd_api_key();
		$icons['zcpg'] = __('Zeno', 'zeno-crypto-checkout-for-easy-digital-downloads');
		return $icons;
	}
);

add_filter('edd_accepted_payment_zeno_image', function () {
	return ZCEDD_PLUGIN_URL . 'assets/images/checkout-logo.png';
});

add_filter('edd_gateway_settings_url_zcpg', function () {
	return admin_url('edit.php?post_type=download&page=edd-settings&tab=gateways&section=zcpg');
});

/**
 * Remove CC form for this gateway (EDD standard pattern).
 */
add_action('edd_' . ZCEDD_GATEWAY_ID . '_cc_form', '__return_false');
 ...
 * @return array
 */
function zc_edd_register_zeno_gateway_section($gateway_sections)
{
	$gateway_sections['zcpg'] = __('Zeno', 'zeno-crypto-checkout-for-easy-digital-downloads');
	return $gateway_sections;
}
add_filter('edd_settings_sections_gateways', 'zc_edd_register_zeno_gateway_section', 1, 1);
 ...
 * @return array
 */
function zc_edd_register_zeno_gateway_settings($gateway_settings)
{

	$secret = edd_get_option('zc_edd_secret_live', '');

	$zeno_settings = array(
		'zc_edd_settings_header' => array(
			'id'   => 'zc_edd_settings_header',
			'name' => '<h3>' . esc_html__('Zeno Crypto Gateway', 'zeno-crypto-checkout-for-easy-digital-downloads') . '</h3>',
			'type' => 'header',
		),

		'zc_edd_api_key_live'    => array(
			'id'   => 'zc_edd_api_key_live',
		...
		'zc_edd_title'           => array(
			...
			'type' => 'text',
			'std'  => esc_html__('USDT, USDC, Binance Pay', 'zeno-crypto-checkout-for-easy-digital-downloads'),
		),
	);
	 ...
	 */
	$zeno_settings = apply_filters('zc_edd_settings', $zeno_settings);

	// Put all Zeno settings under the "zcpg" subsection.
	$gateway_settings['zcpg'] = $zeno_settings;

	return $gateway_settings;
}
add_filter('edd_settings_gateways', 'zc_edd_register_zeno_gateway_settings', 1, 1);

add_filter('edd_is_gateway_setup_zcpg', function ($is_setup) {
	$api_key = zc_edd_api_key();
	return ('' !== $api_key);
});
 ...
 * @return array
 */
function zc_edd_validate_api_key_on_save($input)
{

	$current_key = edd_get_option('zc_edd_api_key_live', '');
	$current_key = is_string($current_key) ? trim($current_key) : '';

	$incoming_key = null; // null = not submitted.
	if (array_key_exists('zc_edd_api_key_live', $input)) {
		$incoming_key = trim((string) $input['zc_edd_api_key_live']);
	}
	...
	$zeno_was_enabled   = edd_is_gateway_active(ZCEDD_GATEWAY_ID);
	$zeno_will_be_enabled = in_array(ZCEDD_GATEWAY_ID, $enabled_gateways, true);
	...
	if (! $zeno_was_enabled && ! $zeno_will_be_enabled) {
		$input['zc_edd_api_key_live'] = $incoming_key; // empty OK.
		return $input;
	}
	...
	$input['zc_edd_api_key_live'] = $incoming_key;

	return $input;
}
add_filter('edd_settings_gateways_sanitize', 'zc_edd_validate_api_key_on_save', 10, 1);


/**
 * Allow redirect to Zeno hosted checkout domain (CRITICAL).
 */
function zc_edd_allow_redirect_to_zeno($hosts)
{
 ...
 * Helpers.
 */
function zc_edd_api_key()
{
	$key = edd_get_option('zc_edd_api_key_live', '');
	return is_string($key) ? trim($key) : '';
}

function zc_edd_secret()
{
	$secret = edd_get_option('zc_edd_secret_live', '');
	return is_string($secret) ? $secret : '';
}

function zc_edd_verification_token($order_id)
{
	$secret = zc_edd_secret();
	if (empty($secret)) {
		return '';
	}
...
function zc_edd_validate_gateway()
{
	$api_key = zc_edd_api_key();
	if (ZCEDD_GATEWAY_ID == edd_get_chosen_gateway() && empty($api_key)) {
		edd_set_error('zc_edd_missing_api_key', esc_html__('Payment gateway is not configured. Please contact site admin.', 'zeno-crypto-checkout-for-easy-digital-downloads'));
	}
}
add_action('edd_pre_process_purchase', 'zc_edd_validate_gateway', 1);

/**
 * Process gateway purchase like EDD PayPal gateway.
 */
function zc_edd_process_purchase($purchase_data)
{
	...
	if (empty($api_key)) {
		...
		edd_set_error('zc_edd_missing_api_key', __('Payment gateway is not configured. Please contact site admin.', 'zeno-crypto-checkout-for-easy-digital-downloads'));
	...
	$verification_token = zc_edd_verification_token($payment_id);
	...
	$response = wp_remote_post(
		trailingslashit(ZCEDD_API_ENDPOINT) . 'api/v1/checkouts',
		array(
			'headers' => array(
				'x-api-key'    => $api_key,
				...
				'X-Client-Version'           => ZCEDD_VERSION,
	...
	if (is_wp_error($response)) {
		...
		edd_set_error('zc_edd_api_error', 'Zeno API WP_Error: ' . $response->get_error_message());
		edd_send_back_to_checkout('?payment-mode=' . $purchase_data['post_data']['edd-gateway']);
		return;
	}
	...
	if (empty($checkout_url)) {
		...
		edd_update_payment_status($payment_id, 'failed');
		edd_set_error('zc_edd_api_error', 'Zeno API Error: Invalid API key');
		edd_send_back_to_checkout('?payment-mode=' . $purchase_data['post_data']['edd-gateway']);
		return;
	}

	edd_update_payment_meta($payment_id, '_zcpg_payment_url', esc_url_raw($checkout_url));

	// IMPORTANT: allow pay.zenobank.io redirect and redirect using EDD helper.
	add_filter('allowed_redirect_hosts', 'zc_edd_allow_redirect_to_zeno', 10, 1);

	edd_redirect($checkout_url);
	exit;
}
add_action('edd_gateway_' . ZCEDD_GATEWAY_ID, 'zc_edd_process_purchase');
